"""Token budget allocation for EXPAND/KEEP/DROP decisions."""
...
def _allocate_greedy(
    ...
    expand_candidates = []
    for span in spans:
        decision = decision_by_id.get(span.span_id, DisclosureDecision(span.span_id, "KEEP"))
        ...
        margin = logits["EXPAND"] - logits["KEEP"]
        trace.append(
            {
    ...
    for span, decision in keep_candidates:
        if budget_used + span.summary_tokens <= token_budget:
            final[span.span_id] = DisclosureDecision(span.span_id, "KEEP", decision.logits, decision.score)
            budget_used += span.summary_tokens
        else:
            final[span.span_id] = DisclosureDecision(span.span_id, "DROP", decision.logits, decision.score)

    for _, span, decision in sorted(expand_candidates, key=lambda item: (-item[0], item[1].span_id)):
        if final.get(span.span_id, DisclosureDecision(span.span_id, "DROP")).action != "KEEP":
            continue
    ...
    for span in spans:
        final.setdefault(span.span_id, DisclosureDecision(span.span_id, "DROP", decision_by_id.get(span.span_id, DisclosureDecision(span.span_id, "DROP")).logits))
...
def _allocate_knapsack(
    ...
    span_by_id = {span.span_id: span for span in spans}
    decision_by_id = {decision.span_id: decision for decision in decisions}
    kept = [action for action in base.final_actions if action.action in {"KEEP", "EXPAND"}]
    ...
    final = [
        DisclosureDecision(action.span_id, "EXPAND" if action.span_id in best_ids else ("KEEP" if action.action == "EXPAND" else action.action), action.logits, action.score)
