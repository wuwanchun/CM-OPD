"""Single-GPU SDFT trainer for corrected memory/tool actions."""
...
def main() -> None:
    ...
    parser.add_argument("--model-path", required=True, help="Local Hugging Face checkpoint path.")
    parser.add_argument("--train-jsonl", required=True, help="JSONL with prompt/label/metadata SDFT records.")
    parser.add_argument("--output-dir", default=str(ROOT / "checkpoints" / "context_policy_sdft"))
    ...
    try:
        ...
        raise SystemExit("torch and transformers are required for local SDFT training.") from exc

    records = read_jsonl(args.train_jsonl)
    if not records:
        raise SystemExit(f"No SDFT training records found: {args.train_jsonl}")
    ...
    for step, record in enumerate(records[:max_steps], start=1):
        ...
        if not torch.isfinite(loss):
            raise SystemExit(f"Non-finite SDFT loss at step {step}: {float(loss.detach().cpu().item())}")
        loss.backward()
        ...
        value = float((loss * grad_accum).detach().cpu().item())
        losses.append(value)
        print(f"sdft_step={step} loss={value:.6f}")
