"""Export review SFT JSONL for inspection; training path uses Slime rollout."""
...
def main() -> None:
    ...
    output = Path(args.output_dir) / f"selection_review_sft_iter{args.iter}_{args.split}.jsonl"
    write_jsonl(output, records)
    print(f"Wrote {len(records)} Review-SFT inspection records: {output}")
